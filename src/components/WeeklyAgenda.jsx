...
import { Calendar, Plus, Check, X, GitCommit, Edit2, Trash2 } from "lucide-react";
...
const WeeklyAgenda = ({ theme, isRetro, toolFleet }) => {
  ...
  useEffect(() => {
    ...
    const weekAgenda = savedAgendas[currentWeek] || [];
  ...
  const saveAgenda = (newAgenda) => {
    const savedAgendas = JSON.parse(localStorage.getItem("weeklyAgendas") || "{}");
    savedAgendas[currentWeek] = newAgenda;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`flex items-center justify-between pb-3 border-b ${theme.border}`}>
        <div>
          <h3 className={`text-lg font-bold uppercase tracking-wide ${theme.textBold}`}>
      ...
      {agenda.length > 0 && (
        <div className="space-y-2">
          <h4 className={`text-xs font-bold uppercase tracking-wider ${theme.textMuted}`}>
            Your Goals
          </h4>
          {agenda.map((goal) => (
            <div
